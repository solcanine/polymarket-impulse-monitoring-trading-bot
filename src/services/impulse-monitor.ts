...
export class ImpulseMonitor {
  ...
  async processCycle(): Promise<void> {
    const enabled = await this.mongodb.getEnabled();
    if (!enabled) return;

    let config = await this.mongodb.getConfig();
    if (!config) {
    ...
    await this.mongodb.appendPricePoint(marketInfo.upTokenId, now, upPrice || 0.5);
    await this.mongodb.appendPricePoint(marketInfo.downTokenId, now, downPrice || 0.5);

    const upHistory = await this.mongodb.getPriceHistory(marketInfo.upTokenId, config.lookbackSec);
    const downHistory = await this.mongodb.getPriceHistory(marketInfo.downTokenId, config.lookbackSec);

    let position = await this.mongodb.getPosition(marketInfo.conditionId);
    ...
    if (!position) {
      if (!alreadyBoughtUp && upPrice > 0 && isSuddenImpulse(upHistory, upPrice, config)) {
        ...
        if (ok) {
          ...
          await this.mongodb.setPosition(marketInfo.conditionId, position);
        }
    ...
    if (position) {
      ...
      if (drop >= trailingStop) {
        ...
        if (!(await this.mongodb.hasBoughtToken(marketInfo.conditionId, hedgeSide))) {
          ...
          if (ok) {
            await this.mongodb.setPosition(marketInfo.conditionId, null);
          }
        } else {
          await this.mongodb.setPosition(marketInfo.conditionId, null);
        }
        ...
        await this.mongodb.setPosition(marketInfo.conditionId, updated);
      }
    }

    await this.mongodb.setImpulseState({
      upPrice,
      downPrice,
